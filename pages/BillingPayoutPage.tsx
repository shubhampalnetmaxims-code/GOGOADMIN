...
import { 
  ...
  MoreVertical, CheckSquare, Square, Navigation
} from 'lucide-react';
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 bg-white p-8 rounded-[32px] border border-gray-100 shadow-sm relative overflow-hidden">
        <div className="flex-1 relative z-10">
          <div className="flex items-center gap-2 mb-1">
             ...
             <h1 className="text-2xl font-black text-gray-900 tracking-tight">Settlements & Billing</h1>
          </div>
          <p className="text-sm text-gray-500 font-medium italic">Monitor driver financial compliance and handle net disbursements.</p>
        </div>
        ...
        <div className="flex items-center gap-3 relative z-10">
           ...
           <div className="bg-amber-50 border border-amber-100 p-4 rounded-2xl flex flex-col min-w-[140px]">
              <span className="text-[10px] font-black uppercase text-amber-400 tracking-widest mb-1">Owed To Fleet</span>
      ...
      <div className="bg-white rounded-[40px] shadow-sm border border-gray-200 overflow-hidden flex flex-col">
        <div className="p-6 bg-gray-50/50 border-b flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="flex gap-3">
             <Button variant="ghost" icon={<Download size={18} />} onClick={exportAll} className="text-xs font-black rounded-xl px-5 uppercase tracking-widest">Full Ledger</Button>
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse min-w-[1000px]">
            <thead>
              <tr className="bg-gray-50/80 text-[10px] font-black text-gray-400 uppercase tracking-widest border-b">
                ...
                <th className="px-4 py-5">Driver Entity</th>
                <th className="px-4 py-5 text-center">Trips</th>
                <th className="px-6 py-5">Gross Earned</th>
              ...
              {filteredDrivers.map(driver => {
                ...
                return (
                  <React.Fragment key={driver.id}>
                    ...
                    >
                      <td className="relative px-8 py-6 w-12 text-center overflow-hidden">
                        {isOverThreshold && (
                      ...
                      <td className="px-4 py-6 text-center">
                         <button 
                          ...
                          className="inline-flex items-center justify-center min-w-[54px] text-xs font-black text-blue-600 px-3 py-1.5 bg-blue-50 hover:bg-blue-100 rounded-xl transition-all shadow-sm"
                         >
                            {driver.completedTrips}
                         </button>
                      </td>
                      <td className="px-6 py-6 text-sm font-black text-gray-900 whitespace-nowrap tracking-tight">${driver.totalEarned.toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>
                      <td className="px-6 py-6 text-sm font-bold text-red-500 whitespace-nowrap tracking-tight">-${driver.totalCommission.toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>
                      <td className="px-6 py-6 text-sm font-black text-blue-600 whitespace-nowrap tracking-tight">
                          ...
                          <div className="bg-white rounded-[32px] border border-gray-100 p-8 shadow-sm">
                            <div className="flex justify-between items-center mb-6">
                               <h4 className="text-[10px] font-black uppercase text-gray-400 tracking-widest flex items-center gap-2">
                                 <History size={14} className="text-blue-500" /> Transaction Audit Stream
                               </h4>
                               <div className="flex items-center gap-4">
                                  <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest bg-gray-50 px-3 py-1 rounded-lg">ID: {driver.id}</p>
                                  <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest bg-gray-50 px-3 py-1 rounded-lg">Aggregate Volume: ${driver.logs.reduce((acc, l) => acc + l.amount, 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}</p>
                               </div>
                            </div>
                            ...
                            {driver.logs.length > 0 ? (
                                ...
                                {driver.logs.map(log => (
                                  <div key={log.id} className="flex items-center justify-between p-5 rounded-2xl border border-gray-100 hover:bg-blue-50/20 transition-all border-l-4 border-l-transparent hover:border-l-blue-600">
                                    <div className="flex items-center gap-5">
                                      ...
                                      <div>
                                        <p className="text-xs font-black text-gray-900 uppercase tracking-tight">{log.type === PayoutType.PAYOUT_TO_DRIVER ? 'Platform Payout' : 'Manual Collection'}</p>
                                    ...
                                    <div className="text-right">
                                      ...
                                      <p className="text-[10px] font-bold uppercase tracking-tighter text-gray-400 mt-1">{log.date} • Verified by {log.adminName}</p>
                                    </div>
                                  </div>
                                {))}
                              </div>
                              ...
                              <div className="text-center py-12 flex flex-col items-center opacity-30">
                                 ...
                                 <p className="text-[10px] font-black uppercase tracking-widest">Zero transactions recorded</p>
                              </div>
                            )}
                          </div>
                        </td>
      ...
      <Modal 
        ...
        title="Fleet Financial Controls"
      >
        <div className="space-y-8">
           <div>
              <label className="text-xs font-black uppercase text-gray-400 tracking-widest block mb-4">Risk Monitoring Threshold</label>
              <div className="bg-gray-50 p-6 rounded-3xl border border-gray-100 mb-6">
                 <div className="flex justify-between items-end mb-6">
                    <div>
                       <p className="text-sm font-black text-gray-900 leading-none">Debt High-Risk Limit</p>
                       <p className="text-[10px] text-gray-400 font-bold uppercase mt-1 tracking-widest">Triggers red highlight indicator</p>
                    </div>
              ...
              <p className="text-[11px] text-gray-500 italic leading-relaxed px-2">
                Rows will be immediately flagged in red if the owned amount exceeds this value by 0.1 or more. 
              </p>
           </div>
           <div className="pt-6 border-t border-gray-100">
              <Button variant="black" fullWidth className="rounded-2xl h-14 uppercase tracking-widest text-[11px] font-black shadow-xl shadow-gray-200" onClick={() => setIsSettingsOpen(false)}>Apply Global Settings</Button>
           </div>
        </div>
      </Modal>
      ...
      <Modal 
        ...
        title="Execute Account Settlement"
      >
        <div className="space-y-6">
          <div className="p-4 bg-gray-50 rounded-2xl border border-gray-100 flex items-center justify-between">
             <div className="flex items-center gap-3">
                ...
                <div>
                   ...
                   <span className="text-[9px] font-bold text-gray-400 uppercase tracking-widest">Registry Balance Check</span>
                </div>
             </div>
             <div className="text-right">
          ...
          <div className="space-y-5">
            <Select 
              ...
              options={[
                { value: PayoutType.PAYOUT_TO_DRIVER, label: 'Platform Disbursement (Payout)' },
                { value: PayoutType.COLLECT_FROM_DRIVER, label: 'Debt Collection (Manual)' }
              ]}
            ...
            <Input 
              ...
              placeholder="e.g. Bank Transfer #9921" 
          ...
          <div className="flex justify-end gap-3 pt-6 border-t border-gray-100">
            <Button variant="secondary" className="rounded-xl px-6" onClick={() => setLogModal({ open: false })}>Cancel</Button>
            <Button variant="black" className="rounded-xl px-8 h-12 font-black uppercase tracking-widest text-[11px] shadow-lg shadow-gray-100" onClick={handleSaveLog}>Authorize & Commit</Button>
          </div>
        </div>
      </Modal>
    </div>
